Criando consultas e funções
Já usamos o Select *; Select nomeColuna, nomeColuna; WHERE; explorar formas de visualização de dados
...
(105, 'Cadeira Gamer Ergonômica', 1200.00, 'Carlos Silva', 'SP', 8200.00),
...
-- Comando para verificar se os 100 registros foram inseridos corretamente
SELECT COUNT(*) AS TotalRegistros FROM Vendas08out;

-- Comando para visualizar uma amostra dos dados
SELECT TOP 10 * FROM Vendas08out;

--SELECTzin que já conhecemos, descrição e preço
SELECT DescricaoProduto, PrecoProduto FROM Vendas08OUT;

--quero melhorar o nome da coluna? comando AS renomeando colunas (aliasing)
SELECT DescricaoProduto AS 'Nome do Produto', PrecoProduto AS 'Preço' FROM Vendas08OUT

--Selecione o nome do produto e seu preço para todos os produtos que custam mais de R$ 2.000,00.
SELECT DescricaoProduto AS 'NOME Produto', PrecoProduto AS 'PREÇO' FROM Vendas08OUT WHERE PrecoProduto > 2000;

--Liste o nome e a renda mensal de todos os vendedores que possuem uma renda mensal inferior a R$ 6.000,00.
SELECT NomeVendedor AS 'nome do colaborador', RendaMensalVendedor AS 'Salário'FROM Vendas08OUT WHERE RendaMensalVendedor < 6000;

--Apresente a lista de todos os produtos, ordenados do mais barato para o mais caro. (ASCENDENTE ou DES decrescente)
SELECT DescricaoProduto, PrecoProduto FROM Vendas08OUT ORDER BY PrecoProduto ASC;

--Mostre todas as vendas, ordenadas pelo nome do vendedor em ordem alfabética.
SELECT * FROM Vendas08OUT ORDER BY NomeVendedor;

--Liste todas as vendas que ocorreram nos estados de 'RJ', 'MG' e 'BA'. (operador IN "EM")
SELECT * FROM Vendas08out WHERE EstadoVendedor IN ('RJ', 'MG', 'BA');

--Encontre todos os produtos cujo preço está entre R$ 400,00 e R$ 800,00. (OPERADOR BETWEEN)
SELECT DescricaoProduto, PrecoProduto FROM Vendas08OUT WHERE PrecoProduto BETWEEN 400 AND 800;

--Encontre todos os produtos que contenham a palavra "Gamer" em sua descrição.
SELECT * FROM Vendas08OUT WHERE DescricaoProduto LIKE '%Gamer%';

-- Liste todos os produtos cuja descrição começa com a palavra "Notebook".
SELECT * FROM Vendas08out WHERE DescricaoProduto LIKE 'Notebook%';

-- Mostre todas as vendas feitas por vendedores cujo sobrenome seja "Silva".
SELECT * FROM Vendas08OUT WHERE NomeVendedor LIKE '% Silva';

--e para ver quantas vendas eu tenho em cada estado??
SELECT
    EstadoVendedor,
...
GROUP BY
    EstadoVendedor
ORDER BY
    QuantidadeDeVendas DESC; -- Opcional: ordena os estados com mais vendas primeiro

--Quantas vendas estão registradas na tabela?
SELECT COUNT(*) AS TotalDeVendas FROM Vendas08OUT;

--Calcule o valor total (soma dos preços dos produtos) que cada vendedor comercializou. Agrupe o resultado pelo nome do vendedor.
SELECT NomeVendedor, SUM(PrecoProduto) AS ValorTotalVendido
FROM Vendas08OUT
GROUP BY NomeVendedor
ORDER BY ValorTotalVendido DESC;

--Conte quantas vendas foram realizadas em cada estado.
SELECT EstadoVendedor, COUNT(ProdutoID) AS QuantidadeDeVendas
FROM Vendas08OUT
GROUP BY EstadoVendedor;

--Qual é o preço médio de todos os produtos cadastrados na tabela?
SELECT AVG(PrecoProduto) AS PrecoMedio FROM Vendas08OUT

--Calcule a renda mensal média dos vendedores para cada estado.
SELECT EstadoVendedor, AVG(RendaMensalVendedor) AS RendaMedia
FROM Vendas08OUT
GROUP BY EstadoVendedor;

-- Encontre o preço do produto mais caro (MAX) e do produto mais barato (MIN) registrados.
SELECT MAX(PrecoProduto) AS ProdutoMaisCaro, MIN(PrecoProduto) AS ProdutoMaisBarato FROM Vendas08OUT
...
--desafio
--Crie um relatório que mostre, para cada estado, o nome do vendedor, a quantidade de vendas que ele realizou e o valor total vendido por ele. 
--Ordene o resultado pelo estado e depois pelo total vendido (do maior para o menor).
SELECT
    EstadoVendedor,
    NomeVendedor,
    COUNT(ProdutoID) AS QuantidadeDeVendas,
...
FROM
    Vendas08out
GROUP BY
    EstadoVendedor, NomeVendedor
ORDER BY
    EstadoVendedor, ValorTotalVendido DESC;



--desafio
-- Agrupe para contar a quantidade de vendas por vendedor
 -- Some o Valor Total das Vendas por Vendedor
--agrupe as vendas por vendedor para ver a quantidade de vendas, 
--o valor total vendido e o ticket médio (preço médio por venda).
--Também ordene o resultado para ver os melhores vendedores no topo.


--Agrupando para Contar a Quantidade de Vendas por Vendedor
SELECT
    NomeVendedor,
    COUNT(ProdutoID) AS QuantidadeDeVendas
FROM
...
--Agrupando para Somar o Valor Total das Vendas por Vendedor
SELECT
    NomeVendedor,
    SUM(PrecoProduto) AS ValorTotalVendido
FROM
...
--Consulta Completa: Juntando Tudo e Ordenando

SELECT
    NomeVendedor,
    COUNT(ProdutoID) AS QuantidadeDeVendas,
    SUM(PrecoProduto) AS ValorTotalVendido,
    AVG(PrecoProduto) AS TicketMedio
FROM
    Vendas08out
GROUP BY
    NomeVendedor
